...
class PedidoServiceImplUnitTest {
    ...
    void crearPedidoDeberiaFallarSiTipoEsObligatorio() {
        ...
        IllegalArgumentException ex = assertThrows(IllegalArgumentException.class, () -> service.crearPedido(dto));
    ...
    void crearPedidoMesaDeberiaConstruirPedidoConDetallesYTotal() {
        ...
        when(pedidoDAO.guardar(any(Pedido.class))).thenAnswer(invocation -> {
            ...
            return new PedidoResponseDTO(1L, pedido.getFecha(), pedido.getEstado().name(), pedido.getTipo().name(), null, 2L, "Mesero", 3L, 8, pedido.getTotal(), List.of(), null);
        });
        ...
        verify(auditoriaService).registrar(eq("CREAR_PEDIDO"), eq("PEDIDO"), any(), contains("50000"), isNull());
    }

    @Test
    void crearPedidoDomicilioDeberiaExigirCliente() {
        PedidoCreateDTO dto = TestDataFactory.pedidoCreateDomicilio(null, 2L, 5L, 1);
        com.marymar.app.TestSupport.TestDataFactory.setField(dto, "clienteId", null);
    ...
    void obtenerOCrearPedidoPorMesaDeberiaRetornarExistenteSiYaHayActivo() {
        ...
        PedidoResponseDTO response = new PedidoResponseDTO(7L, existente.getFecha(), existente.getEstado().name(), existente.getTipo().name(), null, 2L, "Mesero", 3L, 8, existente.getTotal(), List.of(), null);
    ...
    void agregarProductoDeberiaActualizarCantidadYValidarStock() {
        ...
        when(productoRepository.findById(5L)).thenReturn(Optional.of(producto));
        when(pedidoDAO.actualizar(pedido)).thenAnswer(invocation -> {
            Pedido actualizado = invocation.getArgument(0);
            return new PedidoResponseDTO(1L, actualizado.getFecha(), actualizado.getEstado().name(), actualizado.getTipo().name(), null, 2L, "Mesero", 3L, 8, actualizado.getTotal(), List.of(), null);
        });
        ...
        verify(auditoriaService).registrar(eq("AGREGAR_PRODUCTO"), eq("PEDIDO"), eq(1L), contains("Cantidad: 2"), isNull());
    }
